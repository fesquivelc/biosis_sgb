...
public class EjemplarControlador extends Controlador<Ejemplar> {
    ...
    public List<Ejemplar> consultaMultiple(Autor autor, Editorial editorial, Seccion seccion, Tema tema, String tituloISBN, boolean soloDisponibles) {
        StringBuilder sb = new StringBuilder("SELECT e FROM Ejemplar e WHERE (e.libro.titulo LIKE CONCAT('%',:titulo,'%') OR e.libro.isbn10 = :titulo OR e.libro.isbn13 = :titulo)");
        Map<String, Object> params = new HashMap();
        params.put("titulo", tituloISBN);
        if (autor != null) {
        ...
        sb.append(" ORDER BY e.libro.seccion.materia.nombre,e.libro.seccion.nombre,e.libro.titulo ASC");
