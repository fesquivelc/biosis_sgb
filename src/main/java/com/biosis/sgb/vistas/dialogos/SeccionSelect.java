 ...
 * @author Francis
 */
public class SeccionSelect extends javax.swing.JDialog {
    ...
    public SeccionSelect(Component parent, boolean modal) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Seleccione una sección");
        ...
        chkMateria.setBackground(new java.awt.Color(255, 255, 255));
        chkMateria.setFont(ESTILO1);
        chkMateria.setText("Materia");
        chkMateria.setOpaque(false);
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 0;
        gridBagConstraints.gridy = 0;
        pnlBusqueda.add(chkMateria, gridBagConstraints);
        ...
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
        pnlBusqueda.add(cboMateria, gridBagConstraints);
    ...
    private void tblAutorListMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblAutorListMouseClicked
        ...
        if (conteo == 2) {
            ...
            this.seccion = this.seccionList.get(fila);
            this.dispose();
        }
    ...
    private void initComponents2() {
        ...
        this.seccionList = ObservableCollections.observableList(new ArrayList<Seccion>());
        this.materiaList = materiaControlador.buscarTodos();

        //procedemos a bindear
        BindingGroup grupo = new BindingGroup();

        BeanProperty materia = BeanProperty.create("materia.nombre");
        BeanProperty nombre = BeanProperty.create("nombre");
        BeanProperty descripcion = BeanProperty.create("descripcion");
        JTableBinding bindeoTabla = SwingBindings.createJTableBinding(AutoBinding.UpdateStrategy.READ, seccionList, tblAutorList);
        bindeoTabla.addColumnBinding(materia).setColumnName("Materia").setEditable(false);
        bindeoTabla.addColumnBinding(nombre).setColumnName("Sección").setEditable(false);
        bindeoTabla.addColumnBinding(descripcion).setColumnName("Descripción").setEditable(false);
        grupo.addBinding(bindeoTabla);
        
        JComboBoxBinding comboMateria = SwingBindings.createJComboBoxBinding(AutoBinding.UpdateStrategy.READ, materiaList, cboMateria);
        grupo.addBinding(comboMateria);
        
        grupo.bind();
        
        cboMateria.setRenderer(new AbstractListCellRenderer<Materia>(){
            @Override
...
//        String busqueda = txtBusqueda.getText();
        ...
        seccionList.addAll(this.listar(paginaActual, tamanioPagina));

        tblAutorList.packAll();
    }

    private List<Seccion> listar(int pagina, int tamanio) {
        int total = 0;
        Materia materia = null;
        String nombreStr = txtNombre.getText();
        if (this.chkMateria.isSelected()) {
            total = this.seccionControlador.contarXNombreYMateria(nombreStr, materia);
        } else {
            materia = (Materia) cboMateria.getSelectedItem();
            total = this.seccionControlador.contarXNombre(txtNombre.getText());
        }
        ...
        if (this.chkMateria.isSelected()) {
            return this.seccionControlador.buscarXNombreYMateria(txtNombre.getText(), materia, desde, tamanio);
        } else {
            return this.seccionControlador.buscarXNombre(txtNombre.getText(), desde, tamanio);
        }
    ...
    private void checkbox() {
        this.cboMateria.setEnabled(this.chkMateria.isSelected());
    }
